...
const dataController = {
  ...
  async getConsulta(req, res) {
    ...
    try {
      ...
      if (buro) {
        let appliance = await Appliance.find(
          { idBuro: buro[0]._id },
          function (err, docs) {
            if (err) {
              console.log(err);
        ...
        if (appliance) {
          let generalInfo = await GeneralInfo.find(
            { _id: appliance[0].idGeneralInfo },
            function (err, docs) {
              if (err) {
                console.log(err);
                return false;
    ...
    if (user) {
      ...
      if(buroHub.err) {
        return res.status(400).json({
          success: false,
          message: "Error al actualizar el buro",
        });
      }
